...
主要考虑到当前各大AI集成平台都有各自的一些缺点：
cherry-chat,chat-box无法云端多端同步所有配置
lobe-chat本地部署后，知识库云端储存，有空间限制
本项目后期会上线云端配置服务，以自己的电脑当作服务主机。现在仅能提供本地基本运行的服务。

##声明：本项目大部分代码由大语言模型ai完成
...
1.markdown渲染仍有问题
2.    Ai输出完对话得等好一会儿再次返回token数据才能完成结束这一次对话，接下来可以优化成以下方法：
现代大模型接口（如 OpenAI 协议、Anthropic 协议）在结束传输时，并不仅仅是简单地断开连接，而是会发送一个带有 Metadata（元数据）的终结数据包。
机制：当你开启流式输出时，如果请求参数中包含了 stream_options: {"include_usage": true}（OpenAI 规范），API 会在发送完所有文字后，额外补发一个特殊的 Chunk。
内容：这个 Chunk 不包含文字，但包含了 usage 字段（包括 prompt_tokens, completion_tokens, total_tokens）。
感受：因为这个数据包紧随最后一个文字之后，网络传输几乎没有延迟，所以接收到后可以立即显示。

3.置顶出现问题，无法置顶

4.Ai自动重命名对话主题失败

5.用户修改发送的消息后点击发送，发送按钮不会变为停止

6.新对话可以无限创建，应该限制为，如果当前最新对话中没有对话记录，点击新建对话就跳转到这个对话
...
- 提交Issue：[GitHub Issues](https://github.com/vetisn/local-ai-Assistant/issues)
- 邮件：2414644363@qq.com
